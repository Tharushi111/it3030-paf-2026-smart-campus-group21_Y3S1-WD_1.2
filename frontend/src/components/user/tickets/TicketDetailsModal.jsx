...
export default function TicketDetailsModal({ ticket, onClose }) {
  ...
  const imageUrls = (ticket.imageUrls || []).map((img) =>
      ...
      <div className="relative flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-[28px] border border-orange-200 bg-gradient-to-br from-white via-orange-50 to-amber-50 shadow-2xl">
        ...
        <div className="overflow-y-auto px-6 py-6">
          ...
          <div className="mt-6">
            ...
            {imageUrls.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-orange-200 bg-white px-4 py-10 text-center text-sm text-slate-500 shadow-sm">
                No images attached
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
                {imageUrls.map((img, index) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="overflow-hidden rounded-2xl border border-orange-200 bg-white shadow-sm transition hover:shadow-md"
                  >
                    <img
                      src={img}
                      alt={`ticket-${index}`}
